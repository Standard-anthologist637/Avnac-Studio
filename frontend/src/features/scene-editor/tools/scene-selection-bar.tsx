 ...
 *   text node                       → TextFormatToolbar
 *   rect / ellipse / polygon / star → fill swatch + (rect: corner radius)
 *   line                            → stroke color
 *   image                           → type badge + delete
 *   group                           → type badge + ungroup + delete
...
export default function SceneSelectionBar() {
  ...
  if (hasSelection && selectedIds.length === 1) {
    ...
    if (node.type === "ellipse" || node.type === "polygon") {
      const fill = (node as { fill: SaraswatiColor }).fill;
      return (
        <BarShell barRef={barRef} focusMode={focusMode}>
          <FloatingToolbarShell
            role="toolbar"
