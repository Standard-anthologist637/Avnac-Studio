...
export function ensureArrowNoopLayoutRegistered(
  ...
  const registry = (
    mod as typeof mod & {
      classRegistry?: {
        ...
        setClass?: (klass: unknown, className?: string) => void;
      };
    }
  ).classRegistry;
  if (!registry?.setClass) return;
  try {
    if (registry.getClass?.(ArrowNoop.type)) return;
  } catch {
    // Ignore lookup failures and attempt registration below.
  }
  try {
    registry.setClass(ArrowNoop, ArrowNoop.type);
  } catch {
    try {
      registry.setClass(ArrowNoop);
    } catch {
      /* ignore */
    }
  }
 ...
 */
function shaftPathDFromTail(shaftLen: number, bulge: number, t = 0.5): string {
  const x0 = 0;
  const x1 = shaftLen;
  if (bulge === 0) {
 ...
 */
function headTangentAngleDeg(shaftLen: number, bulge: number, t = 0.5): number {
  if (bulge === 0) return 0;
