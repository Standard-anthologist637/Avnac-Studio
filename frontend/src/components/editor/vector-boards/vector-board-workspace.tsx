...
const CURSOR_MOVE = hugeiconToCursorCss(
  Cursor01Icon,
  7,
  2,
  "#1e293b",
...
export default function VectorBoardWorkspace({
  ...
  const fitView = useCallback(() => {
    ...
    const b = normBoundsForSelections(
      ...
      documentRef.current.layers.flatMap((L) =>
        L.visible
          ? L.strokes.map((s) => ({ layerId: L.id, strokeId: s.id }))
          : [],
      ),
    );
    if (!b) {
  ...
  const commitPenBezierDraft = useCallback(
    (closed = false) => {
      ...
      const fill =
        closed && fillColor && fillColor !== "transparent" ? fillColor : "";
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (tool === "move" && penEditSelection) {
      ...
      if (pt && r) {
        ...
        if (stroke?.kind === "pen" && stroke.penAnchors) {
          const hitR =
            8 / (viewRef.current.scale * Math.min(r.width, r.height));
          const hitR2 = hitR * hitR;
      ...
      if (e.shiftKey) {
        nextSelection = alreadySelected
          ? docSelection.filter(
              (s) =>
                !(
                  s.layerId === hitSel.layerId && s.strokeId === hitSel.strokeId
                ),
            )
          : [...docSelection, hitSel];
      } else {
