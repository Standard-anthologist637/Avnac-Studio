...
import ShapeOptionsToolbar from "@/components/editor/shape/shape-options-toolbar";
...
import EditorShortcutsModal from "@/components/editor/dialogs/editor-shortcuts-modal";
...
const FabricEditor = forwardRef<FabricEditorHandle, FabricEditorProps>(
  function FabricEditor(
    ...
    const [bgValue, setBgValue] = useState<BgValue>({
      type: "solid",
      color: "#ffffff",
    });
    ...
    const [selectionShadowUi, setSelectionShadowUi] = useState<FabricShadowUi>(
      DEFAULT_FABRIC_SHADOW_UI,
    );
    ...
    // Selection sync keeps the React toolbar state derived from Fabric rather
    // than trying to maintain duplicate mutable editor state.
    ...
    const openImageCropModal = useCallback(() => {
      ...
      if (!canvas || !mod?.FabricImage || !(a instanceof mod.FabricImage))
        return;
    ...
    // Fabric bootstrapping is isolated here so the dynamic import, canvas
    // lifecycle, and Wails runtime timing stay in one place.
    useLayoutEffect(() => {
      ...
      void (async () => {
        ...
        const onArtboardPointerMove = (opt: {
          target?: FabricObject | null;
        }) => {
    ...
    useEffect(() => {
      ...
      const onTextChanged = (opt: { target: unknown }) => {
        ...
        if (target instanceof mod.Textbox && textboxUsesAutoWidth(target)) {
    ...
    const pasteAvnacClipText = useCallback(
      async (
        text: string,
        atPoint?: { x: number; y: number },
      ): Promise<boolean> => {
    ...
    const pasteFromClipboard = useCallback(
      async (atPoint?: { x: number; y: number }) => {
        const canvas = fabricCanvasRef.current;
        const mod = fabricModRef.current;
        if (!canvas || !mod) return;
        try {
    ...
    // Document capture/apply is the bridge between the imperative Fabric canvas
    // and the rest of the app's persistence and page orchestration.
      ...
      async (doc: AvnacDocumentV1) => {
        ...
        try {
          ...
          try {
            ...
            console.error(
              "FabricEditor: remote image normalization failed",
              err,
            );
          }
    ...
    const undo = useCallback(async (): Promise<boolean> => {
      ...
      const entry = getUndoHistoryEntry({
        snapshots: historySnapshotsRef.current,
        serialized: historySerializedRef.current,
        index: historyIndexRef.current,
      });
    ...
    const redo = useCallback(async (): Promise<boolean> => {
      if (applyingHistoryRef.current) return false;
      const entry = getRedoHistoryEntry({
        snapshots: historySnapshotsRef.current,
        serialized: historySerializedRef.current,
        index: historyIndexRef.current,
      });
      ...
      (boardId: string, doc: VectorBoardDocument) => {
        ...
        queueMicrotask(() => {
          ...
          for (const o of canvas.getObjects()) {
            ...
            const wid = (o as FabricObject & { avnacVectorBoardId?: string })
              .avnacVectorBoardId;
            if (wid === boardId) {
      ...
      const onKey = (e: KeyboardEvent) => {
        ...
        if (e.metaKey || e.ctrlKey) {
          ...
          if (e.key === "a" || e.key === "A") {
            ...
            const objs = c
              .getObjects()
              .filter((o) => o.visible && !getAvnacLocked(o));
            if (objs.length > 0) {
              c.discardActiveObject();
              if (objs.length === 1) {
          ...
          if (!vectorWorkspaceId && (e.key === "c" || e.key === "C")) {
            e.preventDefault();
    ...
    const replaceDocument = useCallback(
      async (doc: AvnacDocumentV1, opts?: { fitViewport?: boolean }) => {
        await applyDoc(doc);
        queueMicrotask(() => {
          if (opts?.fitViewport) {
            zoomUserAdjustedRef.current = false;
    ...
    const loadDocument = useCallback(
      async (file: File) => {
        ...
        if (!doc) return;
        await replaceDocument(doc, { fitViewport: true });
      },
      [replaceDocument],
    );
    ...
    const aiController = useMemo<AiDesignController>(() => {
      ...
      const findById = (id: string): FabricObject | null => {
        ...
        for (const o of objs) {
          if (
            (o as FabricObject & { avnacLayerId?: string }).avnacLayerId === id
          )
            return o;
        }
        return null;
      ...
      const placeObject = (
        ...
        const x =
          typeof spec.x === "number" ? spec.x : origin === "center" ? W / 2 : 0;
        const y =
          typeof spec.y === "number" ? spec.y : origin === "center" ? H / 2 : 0;
        obj.set({
      ...
      return {
        ...
        async addImageFromUrl(spec) {
          ...
          try {
            ...
            console.error(
              "AI addImageFromUrl failed for URL",
              rawUrl.slice(0, 80),
            );
            return null;
          }
    ...
    let elementToolbarAlignAlready: Record<CanvasAlignKind, boolean> | null =
      null;
    ...
    return (
      <div className="relative flex min-h-0 flex-1 flex-col">
        ...
        <div
          ref={viewportRef}
          className="relative flex min-h-0 flex-1 flex-col overflow-auto rounded-2xl bg-[var(--surface-subtle)] [scrollbar-gutter:stable]"
